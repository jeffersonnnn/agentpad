# Slingshot — internal project log (source of truth)

> This is the INTERNAL working log for the Slingshot / agentpad build. It carries the dated
> source-of-truth status, infra notes, on-chain state, and the roadmap. The public, user-facing
> overview lives in [`README.md`](README.md). Read the newest dated section first; each supersedes
> the ones below it. House rule: no em dashes anywhere in this repo's prose.
>
> Security note: this repo is public. Operational secrets (server IP, SSH key names) are redacted
> here and kept in a private ops note. Secrets never live in git (see `.gitignore`).
...
## ✅ LATEST - SOURCE OF TRUTH (2026-09-17)

Read this first. It supersedes every dated section below. No em dashes.

### Since the 2026-09-14 status, the change is design + build-plan, not new production code
The money loop shipped and closed on 2026-09-14 (take-profit/stop-loss rules, one-click claim +
distribution history, realized-PnL + high-water-mark card). Since then the work is a new,
higher-risk capability moving from decision to a gated build:

- **ADR 0006 - Redeemable basket (ACCEPTED 2026-09-14).** A holder can burn (retire to the dead
  address) their agent coin and receive a pro-rata share of the agent's treasury basket: the USDG
  plus the tokenized securities the agent holds (SGOV, SLV, GLD, and any allowed asset). This makes
  the coin a direct claim on the real assets, not only a claim on distributed USDG. It reverses
  ADR 0002's rejection of the redeemable/NAV-backed model, on the owner's report that legal and the
  SEC cleared a redeemable NAV claim. See [`docs/adr/0006-redeemable-basket.md`](docs/adr/0006-redeemable-basket.md).
  This is the highest securities-law exposure in the whole design; it ships testnet-first and is
  off by default per agent (each creator opts in).
- **Redeemer build plan (testnet to mainnet).** A NEW `Redeemer` contract sits BESIDE the settled
  Distributor/FeeSplitter/AgentWallet; it does not change them. Snapshot-epoch redemption (reuse the
  proven distributor pattern), capped to the securities sleeve plus free USDG, never the curve
  reserve. Merkle-proof claim of the ASSETS; the holder's own wallet signs; the server never signs.
  Phases: design lock -> `src/Redeemer.sol` -> `test/Redeemer.fork.t.sol` -> keeper integration ->
  Redeem card + creator switch -> testnet cycle -> external audit -> gated mainnet pilot. About 2 to
  3 weeks to a gated pilot, audit turnaround dependent. Full plan:
  [`docs/REDEEMER-BUILD-PLAN.md`](docs/REDEEMER-BUILD-PLAN.md).
...
- The platform TRADES autonomously, proven on-chain (agent `51363ef5` bought SGOV + SLV through its
  own ERC-4337 account). The money loop is shipped: take-profit/stop-loss, one-click claim +
  distribution history, realized-PnL + HWM card, agent control panel (pause/resume, payout policy,
  withdraw/sweep). 4 live agents. Trades: 2 real. Distributions: 0 (needs a profitable sell above the
  high-water mark). Follows: 0. Reasoning: live on all 4 agents.
- Infra unchanged: DigitalOcean droplet (IP redacted), `/opt/agentpad`, nginx + TLS, 4 pm2 crons
  (`agentpad-web`, `-reasoner` */3, `-grant` */5, `-keeper` */15) plus `agentpad-alerts` */2.
  Same shared Neon Postgres for the box and the laptop.

### Next (priority)
1. **First distribution** - set a low take-profit on agent `51363ef5` (holds SGOV + SLV) and its
   payout mode to `distribute`; a favorable move auto-sells above the high-water mark and the keeper
   pays holders. Still the headline milestone until one epoch publishes.
2. **Redeemer, testnet-first** - execute the build plan Phase 0 onward; do NOT touch mainnet until the
   gate clears.
3. **Fund** the two unfunded agents so more agents trade.
4. **OpenRouter low-balance alert** in the Observatory (prevent silent reasoning stalls).
5. **Standing gates:** rotate `DEPLOYER_KEY`; verify live X posting; security/legal review.

---

## ✅ SOURCE OF TRUTH (2026-09-14)

Read this first. It supersedes the 2026-09-13 section below (infra details there are still accurate:
DigitalOcean droplet `<DROPLET_IP redacted — see internal ops notes>`, `/opt/agentpad`, 4 pm2 crons + agentpad-alerts). No em dashes.
...
`ssh root@<DROPLET_IP redacted — see internal ops notes>` -> `cd /opt/agentpad && git pull` -> for web changes `cd web && npm run build`
...
- The old Hostinger VPS (`<old host, decommissioned>`) **expired and was suspended** by the host. We migrated to a
  **DigitalOcean droplet** and re-pointed DNS. Hostinger is dead; do not use it.
- **Droplet:** name `slingshot`, **IP `<DROPLET_IP redacted — see internal ops notes>`**, NYC1, Ubuntu 24.04, 2 vCPU / 2 GB + 2 GB swap
  (the swap fixed the Next build OOM that plagued the 2 GB Hostinger box). Node 22, pm2, nginx, certbot.
- **SSH:** key-based as `root@<DROPLET_IP redacted — see internal ops notes>` with a key-based login (key names redacted; see internal ops notes).
- **Repo on the box:** `/opt/agentpad`, cloned via a read-only GitHub deploy key (name redacted).
  `git config core.sshCommand` points at the deploy key, so `git pull` just works.
...
- **DNS (Namecheap):** A records `@` and `www` -> `<DROPLET_IP redacted — see internal ops notes>`.
- **Deploy flow:** `ssh root@<DROPLET_IP redacted — see internal ops notes>` -> `cd /opt/agentpad && git pull` -> `cd web && npm run build`
